...
import com.lebrwcd.blog.dao.MessageDao;
...
public class MessageServiceImpl implements MessageService {
    ...
    public int saveMessage(Message message,Message parentMessage) {
        ...
        if(!StringUtils.isEmpty(parentMessage)){
            ...
            String comment = "亲爱的" + parentNickname + "，您在【LeBrWcdの小屋】的评论收到了来自" + nickName + "的回复！内容如下：" + "\r\n" + "\r\n" +  message.getContent();
            String parentEmail = parentMessage.getEmail();

            // 发送邮件
            SimpleMailMessage simpleMailMessage = new SimpleMailMessage();
            simpleMailMessage.setSubject("LeBrWcdの小屋评论回复");  //主题
            simpleMailMessage.setText(comment);   //内容
            simpleMailMessage.setTo(parentEmail); //接收者的邮箱
            simpleMailMessage.setFrom("lebrwcd@163.com");//发送者邮箱
